...
def a_star_planning(sx, sy, gx, gy, ox, oy, reso, rr):
    ...
    # print("nstart",nstart)  #nstart 10,10,0.0,-1
    # print("ngoal",ngoal)  #ngoal 50,50,0.0,-1
    ...
    while True:
        ...
        if abs(current.x - ngoal.x)<=rr and abs( current.y - ngoal.y )<=rr:
            ...
            break
        # Remove the item from the open set, the key aspect of the A_star algorithm the open_set and the closed_set
        del openset[c_id]
        ...
        for i in range(len(motion)):
            node = Node(current.x + motion[i][0],current.y + motion[i][1],current.cost + motion[i][2], c_id)  # x,y,cost, index
...
def calc_obstacle_map(ox, oy, reso, vr): # determine the grids which have been occupied
    ...
    #print("xwidth:", xwidth)  #60
    #print("ywidth:", ywidth)  #60

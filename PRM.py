...
def sample_points(sx, sy, gx, gy, rr, ox, oy, obkdtree):
    ...
    while len(sample_x) <= N_SAMPLE:
        ...
        #print("np.matrix([tx, ty]).T",np.matrix([tx, ty]).T)
        #print("np.matrix([tx, ty])",np.matrix([tx, ty]))
        index, dist = obkdtree.search(np.matrix([tx, ty]).T)            # search in the kdTree

        if dist[0] >= rr:
            sample_x.append(tx)
            sample_y.append(ty)
    # append start and goal point
    sample_x.append(sx)
    sample_y.append(sy)
    sample_x.append(gx)
    sample_y.append(gy)
...
def generate_roadmap(sample_x, sample_y, rr, obkdtree):
    ...
    for (i, ix, iy) in zip(range(nsample), sample_x, sample_y):             # for every (sample_x, sample_y)
        index, dists = skdtree.search( np.matrix([ix, iy]).T, k=nsample)
        inds = index[0][0]
        edge_id = []
        #  print(index)
        for ii in range(1, len(inds)):
            ...
            if len(edge_id) >= N_KNN:                                       # number of edge from one sampled point
                break
...
def PRM_planning(sx, sy, gx, gy, ox, oy, rr):
    ...
    sample_x, sample_y = sample_points(sx, sy, gx, gy, rr, ox, oy, obkdtree)                #随机撒点
    if show_animation:
